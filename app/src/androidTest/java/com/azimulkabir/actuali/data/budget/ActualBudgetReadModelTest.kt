...
class ActualBudgetReadModelTest {
    ...
    fun entityMenuMutationsUpdateRowsAndCrdtLog() = withDatabase { database ->
        ...
        val messages = database.getMessagesSince(com.azimulkabir.actuali.data.sync.HlcTimestamp.ZERO.toString())
    ...
    fun readsEffectivePostableScheduleAndDeduplicatesLinkedTransactions() = withDatabase { database ->
        ...
        assertTrue(schedule.dateCondition is com.azimulkabir.actuali.data.schedules.ScheduleDateCondition.Recurring)
    ...
    fun schedulePosterPostsDueOccurrenceAdvancesAndGatesTheDay() = withDatabase { database ->
        ...
        val poster = com.azimulkabir.actuali.data.schedules.SchedulePoster(
            context, database,
            ActualTransactionWriter(database, "ffffffffffffffff", idFactory = { "posted-schedule" }),
            com.azimulkabir.actuali.data.schedules.ActualScheduleWriter(database, "abababababababab"),
            idFactory = { "posted-schedule" },
        )
        val budgetId = "poster-${UUID.randomUUID()}"
        assertEquals(1, poster.runIfNeeded(budgetId, com.azimulkabir.actuali.data.schedules.DayDate(2026,9,5)))
        ...
        assertEquals(0, poster.runIfNeeded(budgetId, com.azimulkabir.actuali.data.schedules.DayDate(2026,9,5)))
    }
    ...
    fun scheduleListIncludesLifecycleAndComputesPaidState() = withDatabase { database ->
        ...
        assertEquals(com.azimulkabir.actuali.data.schedules.ScheduleAmountOp.EXACT, summary.amountOp)
    ...
    fun scheduleWritePlanPersistsThroughCrdtAndRemainsReadable() = withDatabase { database ->
        val plan = com.azimulkabir.actuali.data.schedules.ScheduleWriteBuilder.create(
            com.azimulkabir.actuali.data.schedules.ScheduleFormFields(
                "Utilities", "store", "checking",
                com.azimulkabir.actuali.data.schedules.ScheduledAmount.Fixed(-2_000),
                date = com.azimulkabir.actuali.data.schedules.ScheduleDateCondition.Fixed(
                    com.azimulkabir.actuali.data.schedules.DayDate(2026,10,1)),
                postsTransaction = false,
            ), "new-schedule", "new-rule", "new-next", 500,
            com.azimulkabir.actuali.data.schedules.DayDate(2026,9,5),
        )
        com.azimulkabir.actuali.data.schedules.ActualScheduleWriter(database, "efefefefefefefef").apply(plan)
